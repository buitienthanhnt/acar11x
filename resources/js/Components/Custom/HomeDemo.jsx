						...
						>
							<div className="w-full">
								<div className="flex gap-x-4">
									<div className="h-20 w-20 bg-red-50 dark:bg-red-800/20 flex items-center justify-center rounded-full">
										<ImagePage source={items.image_path} className={'w-full h-full rounded-full'}></ImagePage>
									</div>
									<div className="self-end flex">
										<span className="font-semibold text-md text-purple-600">{items.updated_at.slice(0, 10)}</span>
									</div>
								</div>
								<h2 className="mt-6 text-xl font-semibold text-gray-900 dark:text-white line-clamp-2">
									{items.title}
								</h2>
								<p className="mt-4 text-gray-500 dark:text-gray-400 text-sm leading-relaxed line-clamp-3">
									{items.desciption}
								</p>
							</div>
							<svg
...
export function HomeTime(params) {
	/**
	* get time of today
			...
			<div className='space-y-3'>
				<div className='flex justify-center'>
					<svg
						...
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						className="h-16 w-auto  dark:bg-gray-900 inline-block"
					>
						<path
				...
				<h3 className='font-bold text-blue-gray-800'>Xin chào: {today}</h3>
			</div>
		</div>
	)
}
