				...
				{bookPaginate.data.map((book) => {
					return (
						<Link href={sprintf(Urls.bookDetail, [book.id])}
							className='grid grid-cols-2 md:grid-cols-1 bg-gray-100 space-y-2 gap-2 p-1 hover:border-1 hover:border-gray-400 hover:shadow-md rounded-md' key={book.id}>
							<img src={book.image_path} alt="image" className='w-full h-auto md:h-60 rounded-md col-span-1 object-cover' />
							<div className='space-y-1 col-span-1'>
								<p className='font-semibold text-base'>{book.name}</p>
								<p className='text-sm font-semibold'>{book.description}</p>
								<p className='font-medium text-blue-500'>{book.price}.000 $</p>
