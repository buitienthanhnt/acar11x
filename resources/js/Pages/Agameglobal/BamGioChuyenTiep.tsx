...
const BamGioChuyenTiep = () => {
    ...
    const onAddUser = (newUser: { name: string }) => {
...
const ClockItem = ({ com, onToggle, active }) => {
    const onPress = useCallback((time: number) => {
        onToggle(time);
    }, [onToggle])

    return (
        <div className={`flex flex-col flex-1 h-72 rounded-md justify-center items-center bg-blue-gray-500 ${active ? 'border-2 border-gray-700' : ''}`}>
            <Clock start={0} autoRun={!!active} type="up" label={com.name} onToggle={onPress}></Clock>
        </div>
    )
}

const UserDialog = ({ open, handleOpen, onSave }) => {
