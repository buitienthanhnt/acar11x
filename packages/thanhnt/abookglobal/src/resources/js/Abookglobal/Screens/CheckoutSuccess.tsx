...
import { useMode, usePageProps } from "@/Pages/Amuaglobal/hooks";
			...
			<div className="flex-1 bg-[url('/ahome/assets/img-1.jpg')] bg-cover bg-center">
				<div className="w-full p-1 md:p-2 space-y-2 container mx-auto">
					...
					<div className="grid md:grid-cols-7 gap-1">
						<div className="bg-gray-200 p-1 md:col-span-2 pl-2">
							<h3 className="font-semibold ml-[-4px]">Thông tin khách hàng:</h3>
							<p>Tên khách hàng: {customer_info.name}</p>
							<p>Số điện thoại: {customer_info.phone}</p>
							<p>email: {customer_info.email}</p>
						</div>
						<div className="bg-gray-200 p-1 md:col-span-2 pl-2">
							<p className="font-semibold ml-[-4px]">Phương thức giao hàng: {shippingMethodSelected.name}</p>
							<p>Tên người nhận: {shipping_address.name}</p>
							<p>Số điện thoại: {shipping_address.phone}</p>
							<p>Địa chỉ: {shipping_address.location}</p>
						</div>
						<div className="bg-gray-200 p-1 md:col-span-3 pl-2">
							<p className="font-semibold ml-[-4px]">Phương thức thanht toán: {payment_method}</p>
...
const TimeInfo = ({ date_from, date_to, selected_time }: { date_from: string, date_to: string, selected_time: string[] }) => {
	...
	if (isDateRangeMode) {
		return (
			<div className="pl-1">
				<p className="font-semibold text-blue-500">Từ ngày: {date_from}</p>
				<p className="font-semibold text-blue-500">Đến ngày: {date_to}</p>
			</div>
		)
	}
	...
	return (
		<div className="flex flex-wrap gap-1 md:gap-2">
			{selected_time.map((item, index) => <p key={index} className="bg-gray-400 p-1 px-2 rounded-md font-semibold text-sm md:text-base">{item}</p>)}
		</div>
	)
}
